...
theorem decodedFullyFixedStep_eq_of_safety
    (ctx : FixedLocalContext cfg ι) (h : LocalSafetyCert ctx) :
...
theorem localCost_nonneg (model : WeightedCostModel) (ctx : FixedLocalContext cfg ι) :
    0 ≤ localStepCost model ctx :=
  localStepCost_nonneg model ctx

theorem jacobiCost_nonneg [Fintype κ]
    (model : WeightedCostModel) (builder : FixedLocalContextBuilder cfg κ η)
    (state : FixedPixelState cfg κ) :
    0 ≤ jacobiStepCost model builder state :=
  jacobiStepCost_nonneg model builder state

theorem fullyFixedLocalCost_le_reference
    (model : WeightedCostModel) (ctx : FixedLocalContext cfg ι)
    (hLookup : model.weight .lookup + model.weight .shift ≤ model.weight .div)
    (hCompare : model.weight .compare ≤ model.weight .div) :
    localStepCost model ctx ≤ referenceLocalStepCost model ctx :=
  fullyFixed_local_cost_le_reference model ctx hLookup hCompare
...
theorem acceptable_of_referenceBudget
    ...
    (hLookup : model.weight .lookup + model.weight .shift ≤ model.weight .div)
    (hCompare : model.weight .compare ≤ model.weight .div) :
    AcceptableFixedFormat model ctx 1 (referenceLocalStepCost model ctx) :=
  ⟨hBudget, fullyFixedLocalCost_le_reference model ctx hLookup hCompare⟩

theorem not_acceptable_of_small_error_budget
    (model : WeightedCostModel) (ctx : FixedLocalContext cfg ι)
    (hBudget : 0 < coefficientErrorBudget cfg) :
    ¬ AcceptableFixedFormat model ctx 0 (referenceLocalStepCost model ctx) :=
  fun hAcc => not_le_of_gt hBudget hAcc.1
