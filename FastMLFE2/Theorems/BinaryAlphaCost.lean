...
theorem localCost_eq_of_alpha_zero
  ...
  simp [FastMLFE2.Core.LocalContext.localCost,
    FastMLFE2.Core.LocalContext.compositingResidual_eq, hα]
    ...
    · have hfg : ctx.normalMatrix.mulVec g 0 = ctx.rhs 0 := by
      ...
      calc
        ctx.totalWeight * foreground g = ctx.rhs 0 := by
          simpa [FastMLFE2.Core.LocalContext.normalMatrix_mulVec_foreground, hα] using hfg
    ...
    · have hbg : ctx.normalMatrix.mulVec g 1 = ctx.rhs 1 := by
      ...
      calc
        (1 + ctx.totalWeight) * background g = ctx.rhs 1 := by
          simpa [FastMLFE2.Core.LocalContext.normalMatrix_mulVec_background, hα] using hbg
  ...
  · rintro ⟨hfg, hbg⟩
    ...
    · calc
        ctx.normalMatrix.mulVec g 0 = ctx.totalWeight * foreground g := by
          simp [FastMLFE2.Core.LocalContext.normalMatrix_mulVec_foreground, hα]
        _ = ctx.foregroundSum := hfg
        _ = ctx.rhs 0 := hrhs0.symm
    · calc
        ctx.normalMatrix.mulVec g 1 = (1 + ctx.totalWeight) * background g := by
          simp [FastMLFE2.Core.LocalContext.normalMatrix_mulVec_background, hα]
