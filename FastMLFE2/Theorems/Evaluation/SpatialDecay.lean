...
theorem decayEnvelope_antitone {C rho : ℝ} (hC : 0 ≤ C) (hrho : 0 ≤ rho) (hrho1 : rho ≤ 1)
    {R S : Nat} (hRS : R ≤ S) :
    C * rho ^ S ≤ C * rho ^ R :=
  mul_le_mul_of_nonneg_left (pow_le_pow_of_le_one hrho hrho1 hRS) hC
...
theorem RadiusDecay.bound_of_radius_le
    ...
    ‖response state₁ p - response state₂ p‖ ≤ h.C * h.rho ^ R :=
  (h.bound p state₁ state₂ S hEqS).trans
    (decayEnvelope_antitone h.hC h.hrho_nonneg h.hrho_lt_one.le hRS)
...
structure HaloDecay {κ : Type*} {α : Type*} [NormedAddCommGroup α]
  ...
  agreeOnHalo : ∀ p Ω state₁ state₂,
    p ∈ Ω → FixedOutside Ω state₁ state₂ →
...
theorem HaloDecay.bound
    ...
    ‖response state₁ p - response state₂ p‖ ≤
      h.radiusDecay.C * h.radiusDecay.rho ^ h.haloWidth p Ω :=
  h.radiusDecay.bound p state₁ state₂ _ (h.agreeOnHalo p Ω state₁ state₂ hp hfix)
...
theorem HaloDecay.bound_of_smaller_radius
    ...
    ‖response state₁ p - response state₂ p‖ ≤ h.radiusDecay.C * h.radiusDecay.rho ^ R :=
  (h.bound hp hfix).trans
    (decayEnvelope_antitone h.radiusDecay.hC h.radiusDecay.hrho_nonneg
      h.radiusDecay.hrho_lt_one.le hR)
