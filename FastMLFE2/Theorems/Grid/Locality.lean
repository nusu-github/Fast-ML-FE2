...
theorem build_eq_of_StateEqOn
    {builder : LocalContextBuilder κ η} {N : Neighborhood κ}
    (hlocal : BuilderLocal builder N) (p : κ)
    {state₁ state₂ : PixelState κ} (hEq : StateEqOn (N p) state₁ state₂) :
...
theorem alphaCenter_le_tau_iff_of_StateEqOn
    {builder : LocalContextBuilder κ η} {N : Neighborhood κ}
    (hlocal : BuilderLocal builder N) (p : κ)
    {state₁ state₂ : PixelState κ} (hEq : StateEqOn (N p) state₁ state₂) (τ : ℝ) :
