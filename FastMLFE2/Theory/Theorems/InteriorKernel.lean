...
noncomputable def validDirEquivDirection4
  ...
  toFun := FastMLFE2.Theory.Canonical.GridPixelData.interiorToValidDir p hInterior
  invFun := fun j => j.1
  left_inv := by
    intro d
    cases d <;> rfl
...
theorem localCtx_rhs_eq_interior_rhs
  ...
  ext i
  fin_cases i
  · simpa [FastMLFE2.Theory.Core.LocalContext.rhs,
      FastMLFE2.Theory.Canonical.GridPixelData.interiorLocalCtx] using
      congrArg
        (fun x =>
...
theorem localCtx_normalMatrix_eq_interior_normalMatrix
  ...
  · rw [FastMLFE2.Theory.Core.LocalContext.normalMatrix,
      ...
      totalWeight_eq_interiorTotalWeight (data := data) (p := p) (state := state)
        (hInterior := hInterior)]
    simp [FastMLFE2.Theory.Canonical.GridPixelData.interiorLocalCtx,
      FastMLFE2.Theory.Canonical.GridPixelData.localCtx]
  · simp [FastMLFE2.Theory.Core.LocalContext.normalMatrix,
      FastMLFE2.Theory.Canonical.GridPixelData.interiorLocalCtx,
      FastMLFE2.Theory.Canonical.GridPixelData.localCtx]
  · simp [FastMLFE2.Theory.Core.LocalContext.normalMatrix,
      FastMLFE2.Theory.Canonical.GridPixelData.interiorLocalCtx,
      FastMLFE2.Theory.Canonical.GridPixelData.localCtx]
