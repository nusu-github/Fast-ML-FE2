...
noncomputable def blurStageTwoCtx (ctx : Ctx ι) : Ctx Unit where
  ...
  fgNeighbor := fun _ => FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx
  bgNeighbor := fun _ => FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx
  epsilonR := 1
  omega := 0
...
noncomputable def blurStageTwoSequentialForeground (ctx : Ctx ι) : ℝ :=
  FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx +
    ctx.alphaCenter * FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual ctx /
      (1 + ctx.alphaCenter ^ 2)
...
@[simp] theorem totalWeight_blurStageTwoCtx (ctx : Ctx ι) :
  ...
  simp [blurStageTwoCtx, FastMLFE2.Theory.Core.LocalContext.totalWeight,
...
@[simp] theorem neighborWeight_blurStageTwoCtx (ctx : Ctx ι) :
  ...
  simp [blurStageTwoCtx, FastMLFE2.Theory.Core.LocalContext.neighborWeight]

@[simp] theorem foregroundMean_blurStageTwoCtx (ctx : Ctx ι) :
    (blurStageTwoCtx ctx).foregroundMean =
      FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx := by
  simp [blurStageTwoCtx, FastMLFE2.Theory.Core.LocalContext.foregroundMean,
    FastMLFE2.Theory.Core.LocalContext.foregroundSum,
    FastMLFE2.Theory.Core.LocalContext.totalWeight,
    FastMLFE2.Theory.Core.LocalContext.neighborWeight,
    FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean]

@[simp] theorem backgroundMean_blurStageTwoCtx (ctx : Ctx ι) :
    (blurStageTwoCtx ctx).backgroundMean =
      FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx := by
  simp [blurStageTwoCtx, FastMLFE2.Theory.Core.LocalContext.backgroundMean,
...
theorem localCost_blurStageTwoCtx_eq_blurStageTwoCost
    ...
    (blurStageTwoCtx ctx).localCost g =
      FastMLFE2.Theory.Approximation.LocalContext.blurStageTwoCost ctx g := by
  simp [blurStageTwoCtx, FastMLFE2.Theory.Core.LocalContext.localCost,
    FastMLFE2.Theory.Core.LocalContext.neighborWeight,
    FastMLFE2.Theory.Approximation.LocalContext.blurStageTwoCost]
  ring
...
theorem foreground_blurStageTwoJointOutput_eq
    ...
    foreground (blurStageTwoJointOutput ctx) =
      FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx +
        ctx.alphaCenter *
          FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual ctx /
  ...
  have hresid :
      ctx.imageValue + ctx.alphaCenter *
          FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx -
          ctx.alphaCenter *
            FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx -
          FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx =
        FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual ctx := by
    rw [FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual_eq]
    ring
  have hbase' :
      foreground (blurStageTwoJointOutput ctx) -
          FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx =
...
theorem abs_blurMeanResidual_le_one_of_boxed
    ...
    (hμF :
      0 ≤ FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx ∧
        FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx ≤ 1)
    (hμB :
      0 ≤ FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx ∧
        FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx ≤ 1)
    (hα : 0 ≤ ctx.alphaCenter ∧ ctx.alphaCenter ≤ 1) :
    |FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual ctx| ≤ 1 := by
  rw [FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual_eq]
  have hlow :
      -1 ≤
        ctx.imageValue -
          (ctx.alphaCenter * FastMLFE2.Theory.Approximation.LocalContext.blurForegroundMean ctx +
            (1 - ctx.alphaCenter) *
              FastMLFE2.Theory.Approximation.LocalContext.blurBackgroundMean ctx) := by
...
theorem abs_blurStageTwoSequential_sub_joint_le_quadratic
  ...
  have hnum_nonneg :
      0 ≤ ctx.alphaCenter * |FastMLFE2.Theory.Approximation.LocalContext.blurMeanResidual ctx| *
