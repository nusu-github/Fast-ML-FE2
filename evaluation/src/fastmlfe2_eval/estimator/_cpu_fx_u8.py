...
from fastmlfe2_eval.estimator._cpu_impl import (
    estimate_multilevel_foreground_background_fx_u8 as _estimate_multilevel_foreground_background_fx_u8,
)
...
__all__ = ["estimate_multilevel_foreground_background_fx_u8"]


def estimate_multilevel_foreground_background_fx_u8(
    ...
    if input_image.dtype != np.uint8:
        msg = f"cpu_fx_u8 backend requires uint8 image, got dtype {input_image.dtype}"
        raise ValueError(msg)
    if input_alpha.dtype != np.uint8:
        msg = f"cpu_fx_u8 backend requires uint8 alpha, got dtype {input_alpha.dtype}"
        raise ValueError(msg)
    ...
    _estimate_multilevel_foreground_background_fx_u8(
        foreground,
        background,
        image_u8,
        alpha_u8,
